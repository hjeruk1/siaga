...
function StudentAvatar({siswa,size='lg'}){
  ...
  if(siswa.foto)return<img src={siswa.foto} alt={siswa.nama} className={`${s} rounded-2xl object-cover flex-shrink-0 border-2 border-white shadow`}/>;
  return<div className={`${s} rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-black flex-shrink-0`}>{siswa.nama[0]}</div>;
}
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end" onClick={onClose}>
      <div className="bg-white rounded-t-3xl w-full max-h-[80vh] flex flex-col" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <div className="font-black text-slate-800">Riwayat Laporan</div>
...
function ReportPanel({siswa,tanggal,onSaved,addToast}){
  ...
  if(!laporan)return<div className="p-6 flex justify-center"><div className="w-6 h-6 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"/></div>;
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="font-black text-slate-800">{siswa.nama}</div>
          <div className="text-xs text-slate-400">{siswa.rombel_nama} · {tanggal}</div>
          <CompleteBadge pct={pct}/>
        </div>
        {saving&&<div className="w-5 h-5 border-2 border-amber-400 border-t-transparent rounded-full animate-spin"/>}
      </div>
        ...
        <div className="flex gap-2">
          {MOOD_OPT.map(o=>(
            <button key={o.v} onClick={()=>tap('mood',o.v)}
              className={`flex-1 py-2.5 rounded-xl font-bold text-sm border-2 transition-all ${laporan.mood===o.v?o.c+' text-white border-transparent':'border-slate-200 text-slate-600 hover:border-amber-300'}`}>
              {o.l}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          {MAKAN_OPT.map(o=>(
            <button key={o.v} onClick={()=>tap('makan',o.v)}
              className={`flex-1 py-2.5 rounded-xl font-bold text-xs border-2 transition-all ${laporan.makan===o.v?o.c+' text-white border-transparent':'border-slate-200 text-slate-600 hover:border-amber-300'}`}>
              {o.l}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          {TIDUR_OPT.map(o=>(
            <button key={o.v} onClick={()=>tap('tidur',o.v)}
              className={`flex-1 py-2.5 rounded-xl font-bold text-sm border-2 transition-all ${laporan.tidur===o.v?o.c+' text-white border-transparent':'border-slate-200 text-slate-600 hover:border-amber-300'}`}>
              {o.l}
            </button>
          ))}
        </div>
          ...
          {AKTIVITAS.map(a=>{
            ...
            return(
              <button key={a} onClick={()=>tapAktivitas(a)}
                className={`px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition-all ${on?'bg-amber-500 text-white border-amber-500':'border-slate-200 text-slate-600 hover:border-amber-300'}`}>
                {a}
              </button>
            );
          })}
      ...
      <div>
        ...
        <button onClick={saveCatatan} className="mt-2 w-full py-2.5 bg-amber-500 text-white font-black rounded-xl text-sm hover:bg-amber-600">
          Simpan Catatan
        </button>
      </div>
      ...
      {formal&&(
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          ...
          <button onClick={copyFormal} className="mt-3 w-full py-2.5 bg-blue-600 text-white font-black rounded-xl text-sm">Copy Laporan</button>
        </div>
      )}
  ...
  return(
    <div className="pb-10">
      <div className="px-4 sm:px-5 pt-4 pb-2">
        {showReminder&&(
          <div className="mb-3 bg-amber-50 border-2 border-amber-400 rounded-2xl px-4 py-3 flex items-center gap-3">
            <span className="text-2xl">⏰</span>
            <div className="flex-1">
              <p className="font-black text-amber-800 text-sm">Pengingat Laporan Harian</p>
              <p className="text-amber-700 text-xs">{incomplete} siswa belum dilaporkan — selesaikan sebelum akhir hari.</p>
            </div>
          </div>
        )}
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
          <input type="date" value={tanggal} onChange={e=>{setTanggal(e.target.value);setSelected(null);}}
            className="text-sm border border-slate-200 rounded-xl px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-amber-400"/>
          <div className="w-full sm:flex-1 bg-slate-100 rounded-full h-2.5 overflow-hidden">
            <div className="bg-emerald-500 h-2.5 rounded-full transition-all" style={{width:siswaList.length?(done/siswaList.length*100)+'%':'0%'}}/>
          </div>
      ...
      {loading?<div className="flex justify-center py-10"><div className="w-8 h-8 border-4 border-amber-400 border-t-transparent rounded-full animate-spin"/></div>:(
        <div className="px-4 sm:px-5 grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-3">
          {siswaList.map(s=>{
            ...
            return(
              <div key={s.siswa_id} className={`bg-white rounded-2xl border-2 overflow-hidden transition-all ${isOpen?'border-amber-400 xl:col-span-2 2xl:col-span-3':'border-slate-100'}`}>
